from random import randint

board = []

for x in range(0, 5):
  board.append(["O"] * 5)

def print_board(board):
  for row in board:
    print (" ".join(row))

print_board(board)

def random_row(board):
  return randint(0, len(board) - 1)

def random_col(board):
  return randint(0, len(board[0]) - 1)

ship_row1 = random_row(board)
ship_col1 = random_col(board)

ship_row2 = random_row(board)
ship_col2 = random_col(board)

numberOfShips = 2

if ship_row2 == ship_row1:
    numberOfShips = numberOfShips -1
    ship_row2 = None
    ship_col2 = None

print("There is/are {} ship(s) to find!".format(numberOfShips))

# Everything from here on should be in your for loop
# don't forget to properly indent!
for turn in range(10):
  print ("Turn", turn + 1)
  guess_row = int(input("Guess Row: "))
  guess_col = int(input("Guess Col: "))

  if numberOfShips == 1:

    if guess_row == ship_row1 and guess_col == ship_col1:
        print ("Congratulations! You sank my battleship!")
        break
    else:
        if guess_row not in range(5) or \
            guess_col not in range(5):
            print ("Oops, that's not even in the ocean.")
        elif board[guess_row][guess_col] == "X":
            print( "You guessed that one already." )
        else:
            print ("You missed my battleship!")
            board[guess_row][guess_col] = "X"
        if (turn == 9):
            print("Game Over")
        print_board(board)
  elif numberOfShips == 2:
      if guess_row == ship_row1 and guess_col == ship_col1:
          print("Congratulations! You sank one of my battleships! Find the next one!")
      if guess_row == ship_row2 and guess_col == ship_col2:
          print("Congratulations! You sank all of my battleships!")
          break
      else:
          if guess_row not in range(5) or \
                          guess_col not in range(5):
              print("Oops, that's not even in the ocean.")
          elif board[guess_row][guess_col] == "X":
              print("You guessed that one already.")
          else:
              print("You missed my battleship!")
              board[guess_row][guess_col] = "X"
          if (turn == 9):
              print("Game Over")
          print_board(board)